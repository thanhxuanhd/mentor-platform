    ...
    const categories = [
        {
            ...
            data: categoryData.create_valid_category,
            expectFn: async () => await categoryPage.expectSuccessCreated()
        },
        {
            label: 'Duplicate Category',
            data: categoryData.create_duplicate_category,
            expectFn: async () => await categoryPage.expectDuplicateFailed()
        },
        {
            label: 'Empty Category Name',
            data: categoryData.create_empty_category_name,
            expectFn: async () => await categoryPage.expectEmptyCategoryNameMessage()
        },
        {
            label: '@Boundary Over length Category Name',
            data: categoryData.create_over_length_category_name,
