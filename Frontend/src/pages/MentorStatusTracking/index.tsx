...
import type { MentorApplicationDetailItemProp, MentorApplicationListItemProp } from "../../types/MentorApplicationType"
...
export default function MentorStatusTrackingPage() {
  ...
  const [applicationHistory, setApplicationHistory] = useState<MentorApplicationListItemProp[]>([])
  ...
  const fetchApplicationDetails = useCallback(async (id: string) => {
    try {
      setLoading(true);
      const response = await mentorApplicationService.getMentorApplicationById(id);
  ...
  const fetchApplications = useCallback(async (id: string) => {
    try {
      setLoading(true);
      const response = await mentorApplicationService.getMentorApplicationByMentorId(id);
  ...
  if (selectedApplication) {
    return (
      <MentorStatusTrackingDetail onBackToList={handleBackToList} application={selectedApplication} />
    )
  }
  ...
  return (
    <div className="bg-gray-800 rounded-lg overflow-hidden shadow-lg p-6">
      ...
      <Card className="border-slate-500/30 backdrop-blur-sm">
        <div className="bg-gradient-to-r from-slate-600 to-slate-650 px-4 py-4 border-b rounded-t-xl border-slate-500/30 -mx-6 -mt-6 mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-white text-lg font-semibold">All Applications</h2>
        ...
        <List
          ...
          renderItem={(mentorApplication) => (
            <List.Item
              ...
              onClick={() => handleViewDetails(mentorApplication.mentorApplicationId)}
            >
              <List.Item.Meta
                avatar={
      ...
      <div className="mt-6 text-center">
        <p className="text-slate-400 text-sm">Click on any application to view detailed status and timeline</p>
      </div>
    </div>
  )
}
