      ...
      >
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1 min-w-0">
            <p
              className={`text-sm font-medium ${type === "success" ? "text-green-800" : type === "error" ? "text-red-800" : "text-blue-800"
                } leading-5`}
            >
              {message}
            </p>
...
const RescheduleModal: React.FC<RescheduleModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    const localDateTime = dayjs(`${newDate.format("YYYY-MM-DD")}T${selectedSlot.startTime}`)
    const utcDateTime = localDateTime.utc()

    if (utcDateTime.isBefore(dayjs().utc())) {
  ...
  const localTimeslots = useMemo(() => {
    if (!availableTimeslots.length || !newDate) return []

    return availableTimeslots.map((slot) => {
      const utcDate = newDate.utc().format("YYYY-MM-DD")
      const { localStartTime, localEndTime } = convertUTCDateTimeToLocal(
        utcDate,
        slot.startTime,
        slot.endTime,
        userTimezone,
        ...
        <div>
          <label style={{ display: "block", marginBottom: "8px", fontWeight: 500 }}>
            ...
            {localTimeslots.length > 0 && (
              <span style={{ color: "#52c41a", marginLeft: "8px" }}>({localTimeslots.length} available)</span>
            )}
          </label>
          <Select
            style={{ width: "100%" }}
...
const ScheduleSession = () => {
  ...
  const convertApiResponseToSession = (apiData: SessionBookingRequest): Session => {
    const hasTimeData = apiData.date && apiData.startTime && apiData.endTime

    let localDate = apiData.date
    let localStartTime = apiData.startTime
    let localEndTime = apiData.endTime

    if (hasTimeData) {
      const converted = convertUTCDateTimeToLocal(apiData.date, apiData.startTime, apiData.endTime, userTimezone)
      localDate = converted.localDate
      localStartTime = converted.localStartTime
      localEndTime = converted.localEndTime
    }
    ...
    return {
      ...
      date: localDate || "N/A",
      startTime: localStartTime || "N/A",
      endTime: localEndTime || "N/A",
      utcDate: apiData.date || "",
      utcStartTime: apiData.startTime || "",
      utcEndTime: apiData.endTime || "",
  ...
  const handleOvertimeSessions = useCallback(async () => {
    ...
    try {
      ...
      const overtimeSessions = sessions.filter((session) => {
        ...
        return sessionDateTime.isBefore(now.utc()) && ["Pending", "Approved"].includes(session.status)
      })
      ...
      if (overtimeSessions.length > 0) {
        ...
        setSessions((prev) => prev.map((session) => updatedSessions.find((u) => u.id === session.id) || session))

        if (updatedSessions.some((s) => s.status === "Cancelled")) {
          showNotification(
            "info",
            `${updatedSessions.filter((s) => s.status === "Cancelled").length} overtime sessions have been cancelled and moved to Past Sessions`,
          )
        }
      }
    } catch (error) {
  ...
  const loadAvailableTimeslots = useCallback(
    async (mentorId: string, utcDate: string) => {
      try {
        setLoadingTimeslots(true)
        const timeslots = await sessionBookingService.getAvailableTimeslotsByDate(mentorId, utcDate)
        const enrichedTimeslots = timeslots.map((slot) => ({
          ...slot,
          startTime: slot.startTime || "00:00",
          endTime: slot.endTime || "00:00",
          date: slot.date || utcDate,
        }))
  ...
  const upcomingSessions = useMemo(
    () =>
      getSessionSessions(["Pending", "Approved", "Rescheduled"]).filter(
        (session) => !dayjs.utc(`${session.utcDate} ${session.utcStartTime}`).isBefore(currentTime.utc()),
      ),
  ...
  const handleRescheduleSubmit = async (sessionId: string, newTimeslotId: string, reason: string) => {
    ...
    try {
      ...
      try {
        const updatedSessionDetails = await sessionBookingService.getSessionDetails(sessionId)

        const { localDate, localStartTime, localEndTime } = convertUTCDateTimeToLocal(
          updatedSessionDetails.date,
          updatedSessionDetails.startTime,
          updatedSessionDetails.endTime,
          userTimezone,
        )
          ...
          prev.map((s) =>
            s.id === sessionId
              ? {
                ...
                startTime: localStartTime,
                endTime: localEndTime,
                utcDate: updatedSessionDetails.date,
                utcStartTime: updatedSessionDetails.startTime,
                utcEndTime: updatedSessionDetails.endTime,
                timeSlotId: updatedSessionDetails.timeSlotId,
        ...
        console.error("Error loading updated session details:", detailsError)
        setSessions((prev) =>
          prev.map((s) =>
            s.id === sessionId ? { ...s, status: "Rescheduled" as const, lastStatusUpdate: dayjs().toISOString() } : s,
          ),
        )
      }
  ...
  const openRescheduleModal = async (session: Session) => {
    try {
      ...
      const { localDate, localStartTime, localEndTime } = convertUTCDateTimeToLocal(
        sessionDetails.date || session.utcDate,
        sessionDetails.startTime || session.utcStartTime,
        sessionDetails.endTime || session.utcEndTime,
        userTimezone,
      )
      ...
      const sessionWithDetails: Session = {
        ...
        startTime: localStartTime,
        endTime: localEndTime,
        utcDate: sessionDetails.date || session.utcDate,
        utcStartTime: sessionDetails.startTime || session.utcStartTime,
        utcEndTime: sessionDetails.endTime || session.utcEndTime,
  ...
  const getBaseColumns = (): ColumnsType<Session> => [
    ...
    },
    {
      ...
      key: "datetime",
      render: (_, record) => {
        const hasValidTime = record.date !== "N/A" && record.startTime !== "N/A" && record.endTime !== "N/A"

        if (!hasValidTime) {
          return (
            <Space direction="vertical" size="small">
              <span style={{ color: "#ff4d4f" }}>
                <ExclamationCircleOutlined /> Time data unavailable
              </span>
              <span style={{ fontSize: "12px", color: "#666" }}>Session ID: {record.id}</span>
            </Space>
          )
        }
        ...
        return (
          <Space direction="vertical" size="small">
            ...
            <span style={{ fontSize: "12px", color: "#666" }}>Timezone: {userTimezone}</span>
          </Space>
        )
      },
    },
